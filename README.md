...
The decoder definition describes the base specifications a decoder like product identifiers, dimensions, electrical limits but also references to products images and manuals.

The firmware definition contains the descriptions of all configuration variables available for the corresponding firmware version. It also contains a structure definition to navigate through the huge amount of variables in an easy way.
...
Changes can be done manually directly in this repository at any time.
...
To be use the new repository with BiDiB-Monitor, the synchronization settings have to be changed. Replace the existing value for the service update url with `https://decoderdb.bidib.org/repository.json`.
...
Therefore the changed file have to be copied from the repository folder to the appropriated folder inside the BiDiB user data directory.
